...
namespace Services
{
    public class OpenExchangeRateService
    {
        ...
        private string ApiKey = "469cf477bda74d23ad165b1ac78c8d62";
        ...
        public static OpenExchangeRateService Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new OpenExchangeRateService();
                }
                return _instance;
            }
        }
        ...
        public async Task<List<OpenAverageExchangeRatesModel>> GetRates()
        {
            ...
            foreach (var Item in results)
            {
                ...
                {
                    var cModel = new OpenAverageExchangeListModel { Country = lItem.Country, Currency = lItem.Currency, Value = lItem.Value };
                    cList.Add(cModel);
                }
        ...
        public async Task<OpenExchangeRatesBaseModel> GetRateByDayRange(string start, string end)
        {
            ...
            try
            {
                ...
                {
                    var response = await httpClient.GetAsync("https://openexchangerates.org/api/time-series.json?app_id=" + ApiKey + "&start=" + start + "&end=" + end + "&base=GBP");
            ...
            catch (HttpRequestException e)
            {
                ...
                var eModel = new ErrorModel { Code = "989", Class = "OpenExchangeRateService Line 48", ErrorMessage = e.Message };
        ...
        public async Task<bool> RebuildRates()
        {
            ...
            int iDay = int.Parse(dates[0]);
            int iMonth = int.Parse(dates[1]);
            int iYear = int.Parse(dates[2]);


            for (int y = 1999; y <= iYear; y++)
            {
                ...
                for (int m = 1; m <= 12; m++)
                {
                    ...
                    {
                        await exRateSrv.GetRateByDayRange(Start, End);
                    }
