...
namespace Services
{
    public class HelpersService
    {
        public List<SaveQueryDateModel> ConvertToTimeDimension(string TimeArray)
        {
            ...
            string[] TimeParts = TimeArray.Split('|');
            foreach (string word in TimeParts)
            {
                ...
                {
                   if(word.Length == 1)
                      Time += GetQuarter(word).ToString() + word.PadLeft(word.Length + 1, '0') + "00";
                   else
                      Time += GetQuarter(word).ToString() + word + "00";
                }
