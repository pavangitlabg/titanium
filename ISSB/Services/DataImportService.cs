...
namespace Services
{
    ...
    {
        private static DataImportService _instance;
        public delegate void CallbackEventHandler(string message);
        public event CallbackEventHandler Callback;
        public event CallbackEventHandler CallbackPercent;
        public int FileMonth;
        public int FileYear;
        public readonly SemaphoreSlim semaphoreReadingZIP = new SemaphoreSlim(1, 1);
        public readonly SemaphoreSlim semaphoreValidation = new SemaphoreSlim(1, 1);

        private string LocalDataPath = "/Users/charlesjardine/Projects/ISSB/ISSB/wwwroot/reports/";


        public static DataImportService Instance
        {
            set { }
            get
            {
                if (_instance == null)
                {
                    _instance = new DataImportService();
                }
                return _instance;
            }
        }
        ...
        public async Task ScanDirectory()
        {
            ...
            Callback?.Invoke("Waiting for files ...");
...
#endif
#if DEBUG
                var RemoveFiles = new DirectoryInfo(LocalDataPath).GetFiles("*.*");
#endif
...
#endif
#if DEBUG
            var uploadedFiles = new DirectoryInfo(LocalDataPath).GetFiles("*.uploading");
#endif
...
#endif
#if DEBUG
            var zipFiles = new DirectoryInfo(LocalDataPath).GetFiles("*.zip");
#endif
            bool bOk = true;
            foreach (var ZipItem in zipFiles)
            {
                await semaphoreReadingZIP.WaitAsync();
                try
                {
                    string[] FileParts = ZipItem.FullName.Split('-');
                    FileMonth = int.Parse(FileParts[2]);
                    FileYear = int.Parse(FileParts[3].Replace(".zip", ""));
#if RELEASE
                    ZipFile.ExtractToDirectory(ZipItem.FullName, mailModel.FilePath);
#endif
#if DEBUG
                    ZipFile.ExtractToDirectory(ZipItem.FullName, LocalDataPath);
#endif
                }
                catch (Exception e)
                {
                ...
                if (bOk)
                {
                    ...
                    Callback?.Invoke("Reading: " + fileName);
                    await ReadFile(fileName, mailModel);
                }
                semaphoreReadingZIP.Release();
            }
        }

        public async Task ReadFile(string Supplier, EmailCredentialsModel mailModel)
        {
            string[] sp = Supplier.Split('-');
            ...
            if (TypeModel.NAME.Equals(model.SearchString) && TypeModel.KEY.Equals("GTT"))
            {
                ...
                var bResult = await ProcessGTT(mailModel, model, (int)BatchNumber);
        ...
        private async Task<bool> ProcessGTT(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
...
#endif
#if DEBUG
            var dataFiles = new DirectoryInfo(LocalDataPath).GetFiles(model.FileExtenstion);
#endif
            ...
            try
            {
                ...
                foreach (var dataFile in dataFiles)
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        {
                            var bMC_GEO = true;
                            string[] words = line.Split(model.Data[0]);
                            ...
                            if (words != null)
                            {
                                ...
                                string[] weightTypes = model.WeightType.Split(',');
                                bool bHasWeight = false;

                                foreach (var wType in weightTypes)
                                {
                                    string sType = wType.ToLower();
                                    if (sType.Contains("kg") || sType.Contains("ki") || sType.Contains("s") || sType.Contains("1"))
                                    {
                                        if (words[9].ToLower().Contains("kg") || words[9].ToLower().Contains("ki") || words[9].ToLower().Contains("s") || words[9].ToLower().Contains("1"))
                                        {
                                ...
                                if (FileMonth == importModel.MONTH && FileYear == importModel.YEAR && bHasWeight && bMC_GEO == true)
                                {
                                    StageingList.Add(importModel);
                                }
                                else if (model.SearchString.Equals("UK"))
                                {
                                    StageingAmendments.Add(importModel);
                                }
                        ...
                        {
                            cnt = 0;
                            Callback?.Invoke("Number of lines Read : " + LinesRead.ToString() + " " + dataFile.FullName);
                        }
                    }
                    ...
                    {
                        Callback?.Invoke("Updateing Amendments for : " + model.SearchString.ToString());
            ...
            {
                var eSrv = new ErrorService();
                var eModel = new ErrorModel { Code = "988", Class = "Error in ProcessGTT line number " + LinesRead + "", ErrorMessage = e.Message };
                await eSrv.UpdateError(eModel);
        ...
        private async Task<bool> ProcessSKorea(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
            var dataFiles = new DirectoryInfo(mailModel.FilePath).GetFiles(model.FileExtenstion);
            ...
            {
                var eSrv = new ErrorService();
                var eModel = new ErrorModel { Code = "988", Class = "DataImportService Line 182", ErrorMessage = e.Message };
                await eSrv.UpdateError(eModel);
        ...
        private async Task<bool> ProcessArgentina(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
            var dataFiles = new DirectoryInfo(mailModel.FilePath).GetFiles(model.FileExtenstion);
            ...
            try
            {
                ...
                foreach (var dataFile in dataFiles)
                {
                    if (dataFile.FullName.Contains("Export"))
                    {
                        sFlow = "E";
                    }

                    if (dataFile.FullName.Contains("Import"))
                    {
                        sFlow = "I";
                    }
                    string[] FileParts = dataFile.FullName.Split('-');

                    int Year = int.Parse(FileParts[2]);
                    int Month = int.Parse(FileParts[3].Substring(0, 2));
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        else
                        {
                            ...
                            int num1;
                            bool res = int.TryParse(words[0], out num1);
                            if (res == true)
                            {
                                importModel.YEAR = int.Parse(words[0]);
                            }
        ...
        private async Task<bool> ProcessMalaysia(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
            var dataFiles = new DirectoryInfo(mailModel.FilePath).GetFiles(model.FileExtenstion);
                ...
                foreach (var dataFile in dataFiles)
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        else
                        {
                            ...
                            int num1;
                            bool res = int.TryParse(words[6], out num1);
                            if (res == true)
                            {
                                importModel.YEAR = int.Parse(words[6]);
                            }
                            ...
                            if (res == true)
                            {
                                importModel.MONTH = int.Parse(words[7]);
                            }
        ...
        private async Task<bool> ProcessBrazil(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
            var dataFiles = new DirectoryInfo(mailModel.FilePath).GetFiles(model.FileExtenstion);
        ...
        private async Task<bool> ProcessCanada(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
            var dataFiles = new DirectoryInfo(mailModel.FilePath).GetFiles(model.FileExtenstion);
                ...
                foreach (var dataFile in dataFiles)
                {
                    ...
                    foreach (var line in lines)
                    {
                        try
                        {
                            ...
                            string[] DataValues = new string[DataSize];
                            foreach (var dataValue in data)
                            {
                                string numericPosition = new string(dataValue.Where(char.IsDigit).ToArray());
        ...
        private async Task<bool> ProcessINDONESIA(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
            ...
            {
                var eSrv = new ErrorService();
                var eModel = new ErrorModel { Code = "988", Class = "Error in line [" + cnt + "] ", ErrorMessage = e.Message };
                await eSrv.UpdateError(eModel);
        ...
        private async Task<bool> ProcessIreland(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            bool bReturn = true;
            var srv = new ValidationEngineService();
                ...
                foreach (var dataFile in dataFiles)
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        if (!line.Contains("Year"))
                        {
                            ...
                            string[] words = strValue.Split(model.Data[0]);
                            ...
                            double weight;
                            if (!double.TryParse(words[8].ToString(), out weight)) weight = 0;

                            importModel.WEIGHT = weight;

                            importModel.MONETARY_VALUE = double.Parse(words[10]);
                            cnt++;
                            if (FileMonth == importModel.MONTH && FileYear == importModel.YEAR)
                                StageingList.Add(importModel);
        ...
        private async Task<bool> ProcessEUROFER(EmailCredentialsModel mailModel, ClientDataImportModel model, int BatchNumber)
        {
            ...
            string[] trailer = tr.Split(',');
            int TrailerSize = trailer.Count();
            string[] TrailerValues = new string[TrailerSize];
                    ...
                    foreach (var line in lines)
                    {
                        if (!line.Substring(0, 1).Equals("T"))
                        {
                            if (line.Substring(0, 1).Equals("1"))
                            {
                                ...
                                {
                                    string numericPosition = new string(headValue.Where(char.IsDigit).ToArray());
                                    var EndPos = int.Parse(numericPosition);
                            ...
                            if (bProsess)
                            {
                                if (line.Substring(0, 1).Equals("2"))
                                {
                                    ...
                                    bool bInsert = true;
                                    foreach (char value in DataValues[1].Trim())
                                    {
                                ...
                                if (line.Substring(0, 1).Equals("3"))
                                {
                                    ...
                                    {
                                        string numericPosition = new string(trailerValue.Where(char.IsDigit).ToArray());
                                        var EndPos = int.Parse(numericPosition);
        ...
        {
            await semaphoreValidation.WaitAsync();
            var bReturn = true;
            var srv = new ValidationEngineService();
            ...
            int percentComplete = (int)Math.Round((100 * complete) / validateList.Count());


            CallbackPercent?.Invoke(percentComplete.ToString());
            counter = 0;
            ...
            foreach (var Item in validateList)
            {
                //Convert Currency
                char pad = '0';
                string ReportDate = Item.YEAR + "-" + Item.MONTH.ToString().PadLeft(2, pad) + "-01";
                if (Item.MONETARY_VALUE > 0)
                {
                    ...
                    if (Item.MONETARY_VALUE.Equals(0))
                    {
                        ...
                        if (errorModel.Code.Equals(string.Empty))
                        {
                            ...
                            errorModel.Type = Data.Enums.DataTypeEnums.CurrencyCode;
                            errorModel.ErrorMessage = "No currency could be calculated for this record " + Item.CURRENCY_CODE;
                            errorModel.Class = "DataImportService";
                            await eService.AddError(errorModel);
                        }
                        Callback?.Invoke("No currency could be calculated for this record " + Item.CURRENCY_CODE);
                        bReturn = false;
                ...
                if (scModel.SOURCE_COUNTRY_ID.Equals(0))
                {
                    ...
                    if (scExceptionModel.SOURCE_COUNTRY_ID.Equals(0))
                    {
                        ...
                        if (errorModel.Code.Equals(string.Empty))
                        {
                            ...
                            errorModel.Type = Data.Enums.DataTypeEnums.SourceCountry;
                ...
                if (mcModel.MARKET_COUNTRY_ID.Equals(0))
                {
                    ...
                    if (mcExceptionModel.MARKET_COUNTRY_ID.Equals(0))
                    {
                        ...
                        if (errorModel.Code.Equals(string.Empty))
                        {
                            ...
                            errorModel.Type = Data.Enums.DataTypeEnums.MarketCountry;
                ...
                try
                {
                    ...
                    {
                        tModel = TariffList.FirstOrDefault(s => s.ShortTariffCode.Equals(Item.H_TARIFF) && s.ReigionCode.Equals(RegionCode.Trim())) ?? new TariffSearchModel { ID = 0 };
                    }
                    else
                    {
                        tModel = TariffList.FirstOrDefault(s => s.LongTariffCode.Equals(Item.IMPORT_TARIFF) && s.SideOfTrade.Equals(Item.SIDE_OF_TRADE) && s.ReigionCode.Equals(RegionCode.Trim())) ?? new TariffSearchModel { ID = 0 };
                        if (tModel.ID == 0)
                        {
                            tModel = TariffList.FirstOrDefault(s => s.LongTariffCode.Equals(Item.IMPORT_TARIFF) && s.SideOfTrade.Equals("B") && s.ReigionCode.Equals(RegionCode.Trim())) ?? new TariffSearchModel { ID = 0 };
                        }
                    }
                }
                catch (Exception e)
                {
                    var eSrv = new ErrorService();
                    var eModel = new ErrorModel { Code = "988", Class = "DataImportService Line 524", ErrorMessage = e.Message };
                    await eSrv.UpdateError(eModel);
                ...
                if (tModel.ID.Equals(0))
                {
                   ...
                   //Chas var hModel = tariffList.FirstOrDefault(s => s.HARMONISED_TARIFF_CODE.Equals(hCode)) ?? new TariffModel { TARIFF_ID = 0 };
                  //Chas  tModel = hModel;
                    ...
                    if (tModel.ID.Equals(0))
                    {
                        ...
                        {
                            await UpdateBatch(BatchNumber, "No Tariff Code found for this record " + Item.IMPORT_TARIFF, true);
                            ...
                            if (errorModel.Code.Equals(string.Empty))
                            {
                                ...
                                errorModel.Type = Data.Enums.DataTypeEnums.TariffCode;
        ...
        public async Task<DataFileImportModel> DoesExitst(string FileName)
        {
            ...
            {
                return new DataFileImportModel { FileName = string.Empty };
            }
