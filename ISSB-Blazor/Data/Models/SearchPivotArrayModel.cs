...
public class SearchPivotArrayModel
{
    public bool ShowWeightZero = true;

    public SearchPivotArrayModel()
    {
        Dates = new List<int>();
    ...
    private int Months(int date)
    {
        return ToYear(date) * 12 + ToMonth(date);
    }
    ...
    private bool Sequential(List<int> dates)
    {
        ...
        foreach (var d in dates.OrderBy(o => o))
        {
            current = Months(d);
    ...
    public PipelineDefinition<BsonDocument, BsonDocument> pipeline()
    {
        ...
        if (Sequential(Dates))
        {
            mDates = new BsonDocument().Add("TIME_ID",
                new BsonDocument().Add("$gte", Dates.Min()).Add("$lte", Dates.Max()));
        }
        else
        {
            aDates = new BsonArray();
            foreach (var date in Dates) aDates.Add(date);
        ...
        foreach (var country in SourceCountryGeoCodes) aSource.Add(country);
        ...
        foreach (var country in MarketCountryGeoCodes) aMarket.Add(country);
        ...
        if (ProductDetail == ProductDetailOption.Long)
        {
            ...
            foreach (var tariff in Products) aTariff.Add(int.Parse(tariff));
        ...
        else
        {
            ...
            var IsTID = false;
            foreach (var tariff in Products)
                if (tariff.Length != 6)
                {
                    IsTID = true;
        ...
        aPort = new BsonArray();
        foreach (var port in Ports) aPort.Add(port);
        ...
        switch (TradeFlow)
        {
            case TradeFlowOption.Imports:
                aFlow.Add("I");
                break;
            case TradeFlowOption.Exports:
                aFlow.Add("E");
                break;
            case TradeFlowOption.Both:
                aFlow.Add("I");
                aFlow.Add("E");
                break;
        }
        ...
        if (IsPivot)
        {
            ...
            foreach (var Item in sort) facetSort.Add(Item.Name.Replace("_id.", ""), "$" + Item.Name);

            facet = new BsonDocument();
            facet.Add("PIVOT", new BsonArray()
                .Add(new BsonDocument()
                    .Add("$bucket", new BsonDocument()
                        .Add("groupBy", "$MONTH")
                        .Add("boundaries", new BsonArray()
                            .Add(0.0)
                            .Add(4.0)
                            .Add(7.0)
                            .Add(10.0)
                            ...
                            .Add("data", new BsonDocument()
                                .Add("$push", new BsonDocument()
                                    .Add("_id", new BsonDocument()
                                        .AddRange(facetSort)
                                        .Add("WEIGHT", "$WEIGHT")
                                        .Add("MONETARY_VALUE", "$MONETARY_VALUE")
                                        .Add("YTD_WEIGHT", "$YTD_WEIGHT")
                                        .Add("YTD_MONETARY_VALUE", "$YTD_MONETARY_VALUE")
                                    )
                                ))
                        )
                    )
            ...
            element++;
            documents[element] = new BsonDocument("$facet", facet);
        }
    ...
    */
    ...
    [Display(Name = "Source Country Geo Codes")]
    public List<string> SourceCountryGeoCodes { get; set; }
    ...
    [Display(Name = "Market Country Geo Codes")]
    public List<string> MarketCountryGeoCodes { get; set; }
    ...
    {
        set => SourceCountryGrouping =
            value.Equals("ViewCountriesSeparately") ? GroupingOption.Separately : GroupingOption.Together;
    }
    ...
    public string TradeFlowType
    {
        set
        {
            switch (value)
            {
                case "SelectImportsOnly":
                    TradeFlow = TradeFlowOption.Imports;
                    break;
                case "SelectExportsOnly":
                    TradeFlow = TradeFlowOption.Exports;
                    break;
                case "SelectBothImportsandExports":
                    TradeFlow = TradeFlowOption.Both;
                    break;
            }
        }
    }
    ...
    public string ProductGroupType
    {
        set
        {
            ...
            {
                case "Broad":
                    ProductDetail = ProductDetailOption.Broad;
                    break;
                case "Medium":
                    ProductDetail = ProductDetailOption.Medium;
                    break;
                case "High":
                    ProductDetail = ProductDetailOption.High;
                    break;
                case "Long":
                    ProductDetail = ProductDetailOption.Long;
                    break;
                case "Custom":
                    ProductDetail = ProductDetailOption.Custom;
                    break;
            }
        }
    ...
    {
        set => ProductGrouping =
            value.Equals("ViewProductsSeparately") ? GroupingOption.Separately : GroupingOption.Together;
    }

    /// <summary>whether to select values option</summary>

    public string TonnesValuesGroup
    {
        set => Values = value.Equals("SelectBothTonnesandValues");
    }
