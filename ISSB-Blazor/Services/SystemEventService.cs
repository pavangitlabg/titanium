...
public class SystemEventService
{
    ...
    public async Task<List<SystemEventLogsModel>> GetSystemEventLogs()
    {
        ...
        while (await cursor.MoveNextAsync())
        {
            ...
            foreach (var document in batch) results.Add(BsonSerializer.Deserialize<SystemEventLogsDB>(document));
        }
    ...
    private void RegisterModels()
    {
        if (!BsonClassMap.IsClassMapRegistered(typeof(UserModel)))
            BsonClassMap.RegisterClassMap<UserModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(AssetModel)))
            BsonClassMap.RegisterClassMap<AssetModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(MarketCountryModel)))
            BsonClassMap.RegisterClassMap<MarketCountryModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SourceCountryModel)))
            BsonClassMap.RegisterClassMap<SourceCountryModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ClientDataImportTransactionsModel)))
            BsonClassMap.RegisterClassMap<ClientDataImportTransactionsModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ClientDataImportModel)))
            BsonClassMap.RegisterClassMap<ClientDataImportModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(DashBoardLayOutModel)))
            BsonClassMap.RegisterClassMap<DashBoardLayOutModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(TradeDataModel)))
            BsonClassMap.RegisterClassMap<TradeDataModel>(cm => { cm.AutoMap(); });
        ...
        if (!BsonClassMap.IsClassMapRegistered(typeof(EmailCredentialsModel)))
            BsonClassMap.RegisterClassMap<EmailCredentialsModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ErrorModel)))
            BsonClassMap.RegisterClassMap<ErrorModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ImportFileTypeModel)))
            BsonClassMap.RegisterClassMap<ImportFileTypeModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(MarketCountryExceptionModel)))
            BsonClassMap.RegisterClassMap<MarketCountryExceptionModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(MarketCountryFilterListModel)))
            BsonClassMap.RegisterClassMap<MarketCountryFilterListModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(MarketCountryFilterModel)))
            BsonClassMap.RegisterClassMap<MarketCountryFilterModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SourceCountryFilterListModel)))
            BsonClassMap.RegisterClassMap<SourceCountryFilterListModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SourceCountryFilterModel)))
            BsonClassMap.RegisterClassMap<SourceCountryFilterModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(PortsModel)))
            BsonClassMap.RegisterClassMap<PortsModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(UserDashboardModel)))
            BsonClassMap.RegisterClassMap<UserDashboardModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ProductAllFilterModel)))
            BsonClassMap.RegisterClassMap<ProductAllFilterModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(RegisterModel)))
            BsonClassMap.RegisterClassMap<RegisterModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ReportDesignerModel)))
            BsonClassMap.RegisterClassMap<ReportDesignerModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SalesProfileModel)))
            BsonClassMap.RegisterClassMap<SalesProfileModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SiteIssuesModel)))
            BsonClassMap.RegisterClassMap<SiteIssuesModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(ReportDesignerModel)))
            BsonClassMap.RegisterClassMap<ReportDesignerModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SiteVisitorModel)))
            BsonClassMap.RegisterClassMap<SiteVisitorModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SourceCountryExceptionModel)))
            BsonClassMap.RegisterClassMap<SourceCountryExceptionModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SourceCountryFilterListModel)))
            BsonClassMap.RegisterClassMap<SourceCountryFilterListModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SourceCountryMappingModel)))
            BsonClassMap.RegisterClassMap<SourceCountryMappingModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SystemControlModel)))
            BsonClassMap.RegisterClassMap<SystemControlModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(TariffModel)))
            BsonClassMap.RegisterClassMap<TariffModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(TariffExceptionsModel)))
            BsonClassMap.RegisterClassMap<TariffExceptionsModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(TariffIndexModel)))
            BsonClassMap.RegisterClassMap<TariffIndexModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(PendingImportHeaderModel)))
            BsonClassMap.RegisterClassMap<PendingImportHeaderModel>(cm => { cm.AutoMap(); });
        if (!BsonClassMap.IsClassMapRegistered(typeof(ErrorModel)))
            BsonClassMap.RegisterClassMap<ErrorModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(EmailMessagesModel)))
            BsonClassMap.RegisterClassMap<EmailMessagesModel>(cm => { cm.AutoMap(); });

        if (!BsonClassMap.IsClassMapRegistered(typeof(SavedQueryModel)))
            BsonClassMap.RegisterClassMap<SavedQueryModel>(cm => { cm.AutoMap(); });
        if (!BsonClassMap.IsClassMapRegistered(typeof(CurrencyCodesModel)))
            BsonClassMap.RegisterClassMap<CurrencyCodesModel>(cm => { cm.AutoMap(); });
    }
}