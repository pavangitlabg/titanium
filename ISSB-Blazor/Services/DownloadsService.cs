...
public class DownloadsService
{
    ...
    public static DownloadsService Instance
    {
        get
        {
            if (_instance == null) _instance = new DownloadsService();
            return _instance;
        }
    }
    ...
    {
        var RecordId = new BsonObjectId(new ObjectId(UserID));
    ...
    public async Task<bool> SaveFile(string Id, string UserId, List<CSVHeaderFieldsModel> fields,
        List<ReportCSVModel> dataList, string ReportName)
    {
        try
        {
#if RELEASE
                ...
                LocalDataPath = "/var/www/build/wwwroot/downloads/";
            ...
            foreach (var dt in dataList)
            {
                ...
                foreach (var f in fields)
                {
                    ...
                    if (prop != null) fLine += prop.GetValue(dt) + ",";
                }
            ...
            foreach (var fItem in lines) sw.WriteLine(fItem);

            sw.Close();

            var SaveToZip = FileName + ".zip";

            if (File.Exists(SaveToZip)) File.Delete(SaveToZip);
            // Thread.Sleep(5000);
            ...
            foreach (var fi in di.GetFiles())
                ...
                if (fi.ToString() == FileName)
                {
                    if (Directory.Exists(LocalDataPath + Id))
                    {
                        ...
                        foreach (var RemoveFile in RemoveFiles) File.Delete(RemoveFile.FullName);
    ...
    public async Task<bool> Add(DownloadsModel model)
    {
        ...
        var UserId = new BsonObjectId(new ObjectId(model.UserId));
        var ReportId = new BsonObjectId(new ObjectId(model.ReportId));
        ...
        {
            var _id = new BsonObjectId(new ObjectId(existsModel._id));
            modelDB._id = _id;
    ...
    {
        var UserId = new BsonObjectId(new ObjectId(model.UserId));
        var RecordId = new BsonObjectId(new ObjectId(model.ReportId));
        ...
        if (results.Count == 0) return new DownloadsModel { Date = DateTime.Now };
