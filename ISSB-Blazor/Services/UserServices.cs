...
public class UserServices
{
    public async Task<UserModel> LoginUser(string email, string password)
    {
        ...
        var filter = Builders<UserDB>.Filter.Eq(x => x.Email, email) &
                     Builders<UserDB>.Filter.Eq(z => z.Password, password);
    ...
    public async Task<UserModel> LoginTokenUser(string email, string token)
    {
        ...
        //TODO Bug of the password encryption
        var filter = Builders<UserDB>.Filter.Eq(x => x.Email, email) &
                     Builders<UserDB>.Filter.Eq(z => z.Password, token);
    ...
    public async Task<UserModel> GetUser(string email)
    {
        ...
        if (results.Count == 0) return new UserModel();
    ...
    public async Task<List<UserModel>> GetUsersAsync()
    {
        ...
        var results = await db.UserDb
            .Find(new BsonDocument())
            .Limit(15)
            .Sort(Builders<UserDB>.Sort.Ascending("UserName"))
            .ToListAsync();
        
        var modelBag = new ConcurrentBag<UserModel>(); // Thread-safe collection
        ...
        Parallel.ForEach(results, item =>
        {
            ...
            modelBag.Add(model); // Add to thread-safe collection
        });
        
        return modelBag.OrderBy(x => x.UserName).ToList();
    }
    ...
    public async Task<UserColourModel> GetUserColours(string _id)
    {
        ...
        var cModel = new UserColourModel
            { _id = Item._id.ToString(), BrandLink = Item.BrandLink, NavBar = Item.NavBar, SideBar = Item.SideBar };
        return cModel;
    }
    ...
    public async Task<UserModel> GetUsersById(string _id)
    {
        ...
        if (Item.AllowedSourceCountries == null) Item.AllowedSourceCountries = new List<SourceCountryDB>();

        if (Item.AllowedTariffCodes == null) Item.AllowedTariffCodes = new List<ProductAllFilterDB>();

        if (Item.AllowedCustomFilters == null) Item.AllowedCustomFilters = new List<ProductCustomFilterDB>();
    ...
    public async Task<bool> UpdateUser(UserModel model)
    {
        ...
        {
            var RecordItemId = new BsonObjectId(new ObjectId(Item._id));
