...
public class QueryService
{
    ...
    {
        var filter = Builders<SavedQueryDB>.Filter.Eq(x => x.User, User) &
                     Builders<SavedQueryDB>.Filter.Eq(z => z.SaveToVisitorPageIndex, true);
        var db = new DbContext();
        var cursor = await db.SavedQueryDb.FindAsync(filter);

        IList<SavedQueryDB> results = cursor.ToList();

        if (results.Count().Equals(0)) return new SavedQueryModel();
    ...
    public async Task<SavedQueryModel> GetReportByID(string User, string Id)
    {
        var RecordId = new BsonObjectId(new ObjectId(Id));
        var filter = Builders<SavedQueryDB>.Filter.Eq(x => x.User, User) &
                     Builders<SavedQueryDB>.Filter.Eq(z => z._id, RecordId);
        var db = new DbContext();
        var cursor = await db.SavedQueryDb.FindAsync(filter);
    ...
    public async Task<SavedQueryModel> GetReportByID(string Id)
    {
        ...
        if (string.IsNullOrEmpty(Record.SelectedCurrency)) Record.SelectedCurrency = "GBP";
    ...
    {
        var filter = Builders<SavedQueryDB>.Filter.Eq(x => x.User, User) &
                     Builders<SavedQueryDB>.Filter.Eq(z => z.Name, Name);
        var db = new DbContext();
        var cursor = await db.SavedQueryDb.FindAsync(filter);
    ...
    public async Task<SavedQueryModel> InserNewQuery(SavedQueryModel model)
    {
        ...
        var filter = Builders<SavedQueryDB>.Filter.Eq(x => x.User, model.User) &
                     Builders<SavedQueryDB>.Filter.Eq(z => z._id, RecordId);
        var db = new DbContext();
        var cursor = await db.SavedQueryDb.FindAsync(filter);
    ...
    public async Task<bool> AddCacheReport(CachedReportsModel model)
    {
        ...
        var bytesSize = modelDB.ToBson().Length;
    ...
    {
        var filter = Builders<SavedQueryDB>.Filter.Eq(x => x.User, User) &
                     Builders<SavedQueryDB>.Filter.Eq(z => z.SaveToDashboard, true);
        var db = DbContext.Instance;
        ...
        foreach (var Record in results)
        {
            ...
            {
                ReportId = "/ReportEngine/SearchResults?Id=" + Record._id,
                Name = Record.Name
            };
