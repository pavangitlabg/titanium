...
public class IndustryService
{
    ...
    public static IndustryService Instance
    {
        ...
        get
        {
            if (_instance == null) _instance = new IndustryService();
            return _instance;
        }
    }
    ...
    public async Task<IndustryUserModel> LoginUser(string email, string password)
    {
        ...
        var filter = Builders<IndustryUserDB>.Filter.Eq(x => x.Email, email) &
                     Builders<IndustryUserDB>.Filter.Eq(z => z.Password, password);
    ...
    public async Task<IndustryUserModel> GetUser(string email)
    {
        ...
        if (results.Count == 0) return new IndustryUserModel();
    ...
    {
        var client = DbContext.Instance._database.Client;
        var database = client.GetDatabase("ISSB_DATA");
        var collection = database.GetCollection<BsonDocument>("IndDataDB");
        ...
        foreach (var Item in yr) bYear.Add(new BsonInt64(Item));
        ...
        foreach (var Item in mt) bMonth.Add(new BsonInt64(Item));

        var bTariff = new BsonArray();

        foreach (var Item in model.TariffCodes) bTariff.Add(new BsonString(Item));

        var bNumber = new BsonArray();

        foreach (var Item in model.FormNumber.OrderBy(x => x.Number).ToList())
            bNumber.Add(new BsonString(Item.Number.TrimEnd()));
        ...
        foreach (var Item in model.FormNumber) bForms.Add(new BsonString(Item.Form));

        var bCols = new BsonArray();

        foreach (var Item in model.ColNumber) bCols.Add(new BsonString(Item));
            ...
            using (var cursor = await collection.AggregateAsync(pipeline, options))
            {
                while (await cursor.MoveNextAsync())
                {
                    ...
                    foreach (var document in batch)
                    {
                        ...
                        if (mod.Datas.Count > 0) data.Add(mod);
        ...
        using (var cursor = await collection.AggregateAsync(pipeline, options))
        {
            while (await cursor.MoveNextAsync())
            {
                ...
                foreach (var document in batch)
                {
                    ...
                    var deserializedCountry = JsonConvert.DeserializeObject<IndustryFormNumbersModel>(json);
    ...
    public async Task<IndustrySavedReportsModel> GetReport(string id)
    {
        ...
        foreach (var code in Item.TCodes) tcodeList.Add(code);

        foreach (var code in Item.ReoprtCols) colsList.Add(code);
