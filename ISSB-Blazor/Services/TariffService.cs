...
public class TariffService
{
    ...
    public async Task<List<TariffModel>> GetTariffsAsync()
    {
        ...
        var results = await db.TariffDb
            .Find(new BsonDocument())
             //.Limit(15)
            .ToListAsync();

        var modelBag = new ConcurrentBag<TariffModel>(); // Thread-safe collection
        ...
        return modelBag.OrderBy(x => x.HARMONISED_TARIFF_CODE).ToList();
    }
    ...
    public async Task<List<string>> GetTariffCodeList()
    {
        ...
        foreach (var item in results) modelList.Add(item.SOURCE_COUNTRY_TARIFF_CODE);
    ...
    public async Task<TariffModel> GetTariffByCode(string product)
    {
        ...
        if (results.Count.Equals(0)) return new TariffModel { SOURCE_COUNTRY_TARIFF_CODE = string.Empty };
    ...
    public async Task<int> GetOnSystemIndex(TariffModel model)
    {
        ...
        var filter = builder.Eq("SOURCE_COUNTRY_TARIFF_CODE", model.SOURCE_COUNTRY_TARIFF_CODE) &
                     builder.Eq("SC_GEO", model.TARIFF_CODE_TABLE_CODE);
        ...
        while (await cursor.MoveNextAsync())
        {
            ...
            foreach (var document in batch) results.Add(BsonSerializer.Deserialize<TradeDataDB>(document));
        }
    ...
    public async Task<TariffExceptionsModel> GetTariffExceptionByCode(string product)
    {
        ...
        if (results.Count.Equals(0)) return new TariffExceptionsModel { SOURCE_COUNTRY_TARIFF_CODE = string.Empty };
    ...
    public async Task<IEnumerable<SelectListItem>> GetTariffIndexLookup()
    {
        ...
        var indexitems = new List<SelectListItem>();
    ...
    public async Task<int> DoesExist(string product, string RegionCode)
    {
        ...
        var filter = builder.Eq("SOURCE_COUNTRY_TARIFF_CODE", product) &
                     builder.Eq("TARIFF_CODE_TABLE_CODE", RegionCode);
    ...
    public async Task<int> GetLastIndex()
    {
        ...
        while (await cursor.MoveNextAsync())
        {
            ...
            foreach (var document in batch) results.Add(BsonSerializer.Deserialize<TariffDB>(document));
        }
