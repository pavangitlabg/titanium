...
{
    public async Task<TradeDataModel> GetTradeDataRecordForAmendments(int Year, int Month, int Source, int Market,
        int Tariff)
    {
    ...
    {
        var words = mirrorBuilder.TariffCodes.Split(',');

        var tCodes = new BsonArray();

        foreach (var code in words) tCodes.Add(code);
        var db = DbContext.Instance;

        var collection = db.TradeDataDb.Database.GetCollection<BsonDocument>("TradeDataDB");
