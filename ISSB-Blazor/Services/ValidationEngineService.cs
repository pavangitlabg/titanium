    ...
    public class ValidationEngineService
    {
        public async Task<bool> AddForValidation(List<PendingImportModel> modelList)
        {
            ...
            foreach (var Item in modelList)
                ...
                {
                    var pad = '0';
                    var H_TARIFF = Item.IMPORT_TARIFF.Substring(0, 6);
                    var IMPORT_TARIFF = Item.IMPORT_TARIFF.TrimEnd();
                    ...
                    var TIME_ID = int.Parse(Item.YEAR.ToString() + QUARTER + Item.MONTH.ToString().PadLeft(2, pad) +
                                            "00");
                ...
                catch (Exception e)
                {
                    ...
                    var eModel = new ErrorModel
                        { Code = "989", Class = "ValidationEngineService Line 77", ErrorMessage = e.Message };
        ...
        public async Task<bool> AddForAmendments(List<PendingImportModel> modelList)
        {
            ...
            foreach (var Item in modelList)
                ...
                catch (Exception e)
                {
                    var eSrv = new ErrorService();
                    var eModel = new ErrorModel
                        { Code = "989", Class = "AddForAmendments Line 147", ErrorMessage = e.Message };
                    await eSrv.UpdateError(eModel);
                }
        ...
        public async Task<bool> AddAfterValidation(List<PendingImportModel> modelList)
        {
            ...
            foreach (var Item in modelList)
                ...
                catch (Exception e)
                {
                    var eSrv = new ErrorService();
                    var eModel = new ErrorModel
                        { Code = "989", Class = "ValidationEngineService Line 128", ErrorMessage = e.Message };
                    await eSrv.UpdateError(eModel);
                }
        ...
        public async Task<YTD_MODEL> CalculateYTD(PendingImportModel model)
        {
            ...
            try
            {
                var szStartTimeId = model.YEAR + "10100";
                var SearchTimeID = int.Parse(szStartTimeId);
            ...
            catch (Exception e)
            {
                var eSrv = new ErrorService();
                var eModel = new ErrorModel
                    { Code = "988", Class = "ValidationEngineService Line 427", ErrorMessage = e.Message };
                await eSrv.UpdateError(eModel);
            }
