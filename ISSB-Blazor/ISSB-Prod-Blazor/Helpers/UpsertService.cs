...
public class UpsertService(DataImportService dataImportService)
{
    
    private readonly SemaphoreSlim semaphoreAmendments = new(1, 1);
    ...
    public void PostAmendments(int BatchNumber, string GeoCode)
    {
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                {
                    var TradeModel = await tradeSrv.GetTradeDataRecordForAmendments(Item.YEAR, Item.MONTH,
                        Item.SOURCE_COUNTRY_ID, Item.MARKET_COUNTRY_ID, Item.TARIFF_ID);
                    if (TradeModel.BATCH_NO != 0)
                    {
                        ...
                        var Changed = Item.WEIGHT + "|" + Item.MONETARY_VALUE;
                ...
                var status = "Batch: " + PostBatchNumber + " UK Additions";
                ...
                await dataImportService.ProcessValidation((int)PostBatchNumber, "CN");
            }
            catch (Exception e)
            {
                ...
                var eModel = new ErrorModel
                    { Code = "70", Class = "UpsertService Code Line 70", ErrorMessage = e.Message };
    ...
    public async Task<List<PendingImportModel>> GetPendingRecordsTopOne(int BatchNo)
    {
        ...
        var options = new FindOptions<PendingImportDB> { Limit = 1 };
    ...
    public async Task<List<PendingImportModel>> GetHowlerRecords(int BatchNo)
    {
        ...
        var WeightLimit = cntl.HowlerWeight;
        var ValueLimit = cntl.HowlerValue;

        var builder = Builders<PendingImportDB>.Filter;
        var filter = builder.Eq("BATCH_NO", BatchNo) & builder.Gte("WEIGHT", WeightLimit) &
                     builder.Gte("MONETARY_VALUE", ValueLimit);
    ...
    public async Task<bool> UpsertToProduction(int BatchNo)
    {
        ...
        try
        {
            ...
            if (resultsCheck.Count.Equals(1)) return false;
            ...
            foreach (var Item in results)
            {
                ...
                if (match == null) scList.Add(Item.SC_GEO);
